...
def plot_segments_similarity(
    image: np.ndarray, masks: torch.Tensor, similarities: np.ndarray
) -> None:
    image = torchvision.transforms.ToTensor()(image)
    colors = similarities_to_rgb(similarities, "viridis")
    img_with_segmentations = torchvision.utils.draw_segmentation_masks(
        image, masks, colors=colors
    )
    plt.axis("off")
    plt.imshow(F.to_pil_image(img_with_segmentations))
...
def plot_bbox(image: np.ndarray, bbox: torch.Tensor) -> None:
    ...
    np.random.seed(42)
    random_colors = [
        (r, g, b) for r, g, b in (255 * torch.rand((bbox.shape[0], 3))).int()
    ]
    img_with_bbox = torchvision.utils.draw_bounding_boxes(
        image, bbox, colors=random_colors, width=3
    )
    plt.axis("off")

...
class ImageCaptioner:
    def __init__(self, system_prompt: str, user_query: str, max_images: int, tag: bool, bg_mask_mode: str, 
                 allow_no_object_pred: bool, no_object_prompt: str = NO_OBJECT_MSG, force_reponse_prefix: bool = True, prefix: str = RESPONSE_PREFIX):
        ...
        if bg_mask_mode not in BG_MODE_PROMPTS:
            raise ValueError(f"Invalid bg_mask_mode. Please select value in {BG_MODE_PROMPTS.keys()}.")
        ...
        if self.force_response_prefix:
            self.full_system_prompt += " " + f"Begin your response with '{self.prefix}'."

        if self.tag:
            log.info(f"Instantiated Image Tagger with full system prompt: {self.full_system_prompt}")
        else:
            log.info(f"Instantiated Image Captioner with full system prompt: {self.full_system_prompt}")
    ...
    def caption_map(self, map: "ObjectMap") -> None:
        ...
        for obj in pbar:
            ...
            imgs = [self.preprocess_image(view, mask) for (view, mask) in zip(views, masks)]
